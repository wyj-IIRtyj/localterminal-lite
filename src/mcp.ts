...
export function createMcpServer(service: ExtensionFacade): McpServer {
  const server = new McpServer({ name: 'localterminal-lite', version: '1.0.1' }, {
    instructions: [
      ...
      'Pass identity={sessionId,sessionToken} on authenticated calls. Session state has highest priority: the final LocalTerminal call of every work turn must be session_checkpoint with the accurate phase.',
