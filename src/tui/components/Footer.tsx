...
function hints(tab: number, detail: Detail | undefined, zh: boolean): string {
  ...
  if (tab === 0) return zh ? '1–7/Tab 页面   滚轮/↑↓ 滚动   c 配置   v 凭据   q 退出' : '1–7/Tab page   wheel/↑↓ scroll   c configure   v credentials   q quit';
  ...
  if (tab === 5) return zh ? '滚轮/↑↓ 滚动   c 修改配置   v 显示凭据   k 轮换凭据   q 退出' : 'wheel/↑↓ scroll   c configure   v reveal   k rotate   q quit';
