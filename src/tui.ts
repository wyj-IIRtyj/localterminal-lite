...
export class LiteTui {
  ...
  private readonly freeScrollTabs = new Set<number>();
  ...
  private cardViewport(cards: string[][], selected: number, height: number): string[] {
    ...
    if (this.freeScrollTabs.has(this.tab)) {
      const anchor = Math.min(Math.max(0, total - 1), scroll + Math.max(0, Math.floor(height / 3)));
      const visibleFocus = cards.findIndex((card, index) => (offsets[index] || 0) <= anchor && (offsets[index] || 0) + card.length > anchor);
      if (visibleFocus >= 0) this.selected[this.tab] = visibleFocus;
    } else {
